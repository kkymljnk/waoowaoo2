...
export async function handleEpisodeSplitTask(job: Job<TaskJobData>) {
    ...
    for (let attempt = 1; attempt <= MAX_EPISODE_SPLIT_ATTEMPTS; attempt += 1) {
      try {
        ...
        await reportTaskProgress(job, 60, {
          ...
          stageLabel: attempt === 1 ? 'Parsing episode split results' : `Parsing episode split results（重试 ${attempt - 1}）`,
          displayMode: 'detail',
        })
        ...
        await reportTaskProgress(job, 80, {
          ...
          stageLabel: 'Matching episode content range',
          displayMode: 'detail',
        })
        ...
        for (let idx = 0; idx < splitEpisodes.length; idx += 1) {
          ...
          if (!startMarker || !endMarker) {
            throw new Error(`episode_${idx + 1} must provide both startMarker/endMarker`)
          }

          const startMatch = markerMatcher.matchMarker(startMarker, searchFrom)
          if (!startMatch) {
            throw new Error(`episode_${idx + 1} startMarker cannot locate`)
          }
          const endMatch = markerMatcher.matchMarker(endMarker, startMatch.endIndex)
          if (!endMatch) {
            throw new Error(`episode_${idx + 1} endMarker cannot locate`)
          }
          ...
          if (rawStartIndex !== null && Math.abs(rawStartIndex - startMatch.startIndex) > 200) {
            throw new Error(`episode_${idx + 1} startIndex marker deviation too large`)
          }
          ...
          if (rawEndIndex !== null && Math.abs(rawEndIndex - endMatch.endIndex) > 200) {
            throw new Error(`episode_${idx + 1} endIndex marker deviation too large`)
          }
          ...
          if (startPos < searchFrom || endPos <= startPos || endPos > content.length) {
            throw new Error(`episode_${idx + 1} boundary range invalid`)
          }
  ...
  await reportTaskProgress(job, 96, {
    ...
    stageLabel: 'Smart episode split complete',
    displayMode: 'detail',
  })
