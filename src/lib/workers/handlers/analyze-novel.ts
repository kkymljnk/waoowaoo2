...
export async function handleAnalyzeNovelTask(job: Job<TaskJobData>) {
  ...
  const characterPromptTemplate = buildPrompt({
    ...
    variables: {
      ...
      characters_lib_info: charactersLibName || 'N/A',
    },
  })
  ...
  await reportTaskProgress(job, 70, {
    ...
    stageLabel: 'Scene analysis complete',
    displayMode: 'detail',
    stepId: 'analyze_locations',
    stepTitle: 'Scene analysis',
    stepIndex: 2,
  ...
  for (const item of parsedLocations) {
    ...
    const invalidKeywords = ['Fantasy', 'Abstract', 'Unclear', 'Spatial anchor', 'Not specified', 'Unclear']
