...
export async function handleAssetHubAIModifyTask(job: Job<TaskJobData>) {
  ...
  const finalPrompt = isCharacter
    ...
    : buildPrompt({
      ...
      locale: job.data.locale,
      variables: {
        location_name: readRequiredString(payload.locationName || 'Scene', 'locationName'),
    ...
    async () =>
      await executeAiTextStep({
        ...
        meta: {
          ...
          stepTitle: isCharacter ? 'Character description modification' : 'Scene description modification',
          stepIndex: 1,
          stepTotal: 1,
        },
      }),
