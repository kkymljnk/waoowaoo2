...
export async function handleScreenplayConvertTask(job: Job<TaskJobData>) {
  ...
  const charactersLibName = novelData.characters.map((item) => item.name).join('、') || 'N/A'
  const locationsLibName = novelData.locations.map((item) => item.name).join('、') || 'N/A'
  const charactersIntroduction = buildCharactersIntroduction(novelData.characters)

  await reportTaskProgress(job, 10, {
    stage: 'screenplay_convert_prepare',
    stageLabel: 'Preparing script conversion parameters',
    displayMode: 'detail',
  })
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    const stepTitle = `ClipScript转换 ${stepIndex}/${total}`
    ...
    await reportTaskProgress(job, progress, {
      ...
      stageLabel: 'Executing script conversion',
      displayMode: 'detail',
      message: stepTitle,
      stepId,
      stepTitle,
  ...
  await reportTaskProgress(job, 96, {
    ...
    stageLabel: 'Script conversion results saved',
    displayMode: 'detail',
    message: `完成 ${successCount}/${total} 个Clip`,
  })
