...
export async function runStoryToScriptOrchestrator(
  ...
  const baseCharactersText = baseCharacters.length > 0 ? baseCharacters.join('、') : 'N/A'
  const baseLocationsText = baseLocations.length > 0 ? baseLocations.join('、') : 'N/A'
  ...
  onLog?.('开始步骤1：Character/Scene analysis（并行）')
  const [
  ...
  // 合并新发现Character与已有Character库（新Character优先，已有Character补充），避免已有Character被覆盖丢失
  ...
  // 合并Character介绍：新Character + 未被新Character覆盖的已有Character介绍
  ...
  onLog?.('开始步骤2：Clip segmentation（最多重试1次）', {
    charactersLibName,
    locationsLibName,
  })
  ...
  const splitPromptBase = applyTemplate(promptTemplates.clipPromptTemplate, {
    input: content,
    locations_lib_name: locationsLibName || 'N/A',
    characters_lib_name: charactersLibName || 'N/A',
    characters_introduction: charactersIntroduction || '暂No characters介绍',
  })
  ...
  onLog?.('开始步骤3：对每个Clip做Script转换（并行）', { clipCount: clipList.length })
