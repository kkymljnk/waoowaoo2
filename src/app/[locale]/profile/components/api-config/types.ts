 ...
 * @returns 解析后的 { provider, modelId }，如果N/A法解析返回 null
 */
...
/**
 * 检查一个复合 Key 是否匹配指定的Model
 * @param key - 复合 Key（provider::modelId）
 * @param provider - 目标厂商 ID
 * @param modelId - 目标Model ID
 * @returns 是否匹配
 */
...
export interface TutorialStep {
    text: string           // 步骤Description (i18n key)
    url?: string           // 可选的链接地址
}
...
/**
 * 获取 Google 官方Model列表的克隆副本，provider 替换为指定 ID。
 * 用于 gemini-compatible 新增时自动预设Model。
 * 排除 batch Model（Google 特有的异步批量处理）。
 */
