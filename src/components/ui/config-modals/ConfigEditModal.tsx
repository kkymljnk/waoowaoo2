...
export function SettingsModal({
    ...
    /**
     * 切换Model时，自动将该Model所有 capability fields 的第一个 option 写入 overrides
     * 解决 UI 视觉上显示Default选中（第一项高亮）但 DB 实际为空，导致 requireAllFields 报错的问题
     */
    const handleModelChange = (
        ...
        // 用新选中的Model的 capabilities 计算 fields，而不是旧Model的
        ...
        const existing = isRecord(nextOverrides[modelKey])
            ? { ...(nextOverrides[modelKey] as Record<string, CapabilityValue>) }
            : {}
        // 只对尚未配置的 field SettingsDefault值（不覆盖已有配置）
        let changed = false
